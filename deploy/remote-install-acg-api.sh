...
find_hoarfrost_nginx_conf() {
  local f

  for f in /etc/nginx/sites-enabled/* /etc/nginx/conf.d/*.conf; do
    [ -f "$f" ] || continue
    if grep -Eq 'server_name[[:space:]]+([^;[:space:]]+[[:space:]]+)*hoarfrost\.top([[:space:];]|$)' "$f" 2>/dev/null; then
      printf '%s\n' "$f"
      return 0
    fi
  done
...
SECURITY_CONF="/etc/nginx/conf.d/hoarfrost-security-zones.conf"
SECURITY_TMP=$(mktemp)
...
SNIP="/etc/nginx/snippets/hoarfrost-acg-api.conf"
SNIP_TMP=$(mktemp)
cat >"$SNIP_TMP" <<'NGX'
...
location /cos/ {
    proxy_pass https://my-blog-static-1464122491.cos.ap-guangzhou.myqcloud.com/;
    proxy_ssl_server_name on;
    proxy_set_header Host "my-blog-static-1464122491.cos.ap-guangzhou.myqcloud.com";
...
CONF="$(find_hoarfrost_nginx_conf)"

if [ -n "$CONF" ] && ! grep -q 'hoarfrost-acg-api.conf' "$CONF"; then
  run_sudo sed -i '/server_name.*hoarfrost\.top/a\    include snippets/hoarfrost-acg-api.conf;' "$CONF"
fi
