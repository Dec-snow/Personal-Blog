...
- Requests target `https://hoarfrost.cloud/api/chat` by default.
- Each request uses the fixed prompt: `请回复 OK，并说明当前服务正常`.
- Requests identify themselves with `User-Agent: hoarfrost-ai-healthcheck`.
- The script reads an optional login cookie from `HOARFROST_AI_COOKIE` so the checks can use the logged-in quota.
