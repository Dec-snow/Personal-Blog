...
type ownerAssetUploader interface {
	UploadImage(ctx context.Context, kind, album, filename, mimeType string, body []byte) (ownerCOSUploadResult, error)
...
func (u ownerTencentCOSUploader) UploadImage(ctx context.Context, kind, album, filename, mimeType string, body []byte) (ownerCOSUploadResult, error) {
	...
	if _, err := u.client.Object.Put(ctx, objectKey, bytes.NewReader(body), opt); err != nil {
...
func (u ownerTencentCOSUploader) DeleteImage(objectKey string) error {
...
func ownerCOSObjectKey(kind, album, filename string) string {
	...
	if kind == "moment" {
		now := time.Now().UTC()
		return path.Join("随笔", now.Format("2006"), now.Format("01"), filename)
	}
	now := time.Now().UTC()
...
// ownerCOSObjectKeyFromURL extracts the COS object key from a full public URL.
// Returns empty string if the URL is not from our configured COS bucket.
func ownerCOSObjectKeyFromURL(cfg ownerCOSConfig, imageURL string) string {
	if cfg.baseURL == "" {
		return ""
	}
	...
	// Handle /cos/ proxy paths
	if strings.HasPrefix(raw, "/cos/") {
		escapedPath := strings.TrimPrefix(raw, "/cos/")
	...
	// Handle full COS URLs
	base := strings.TrimRight(cfg.baseURL, "/") + "/"
	if strings.HasPrefix(raw, base) {
		escapedPath := strings.TrimPrefix(raw, base)
		parts := strings.Split(escapedPath, "/")
		for i, part := range parts {
			if unescaped, err := url.PathUnescape(part); err == nil {
				parts[i] = unescaped
...
// isOurCOSImage checks whether the given URL belongs to our configured COS bucket.
func isOurCOSImage(imageURL string) bool {
	cfg, err := loadOwnerCOSConfig()
	if err != nil {
		return false
	}
	return ownerCOSObjectKeyFromURL(cfg, imageURL) != ""
}
