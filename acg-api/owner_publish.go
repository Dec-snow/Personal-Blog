...
const (
	...
	ownerPublishDefaultOwner   = "Dec-snow"
	ownerPublishDefaultRepo    = "hoarfrost"
...
// ownerGitHubFile represents a file fetched from the GitHub Contents API.
...
// getGitHubFile fetches a file from the GitHub repository via the Contents API.
func (p *ownerGitHubPublisher) getGitHubFile(path string) (ownerGitHubFile, error) {
	...
	if payload.SHA == "" {
		return ownerGitHubFile{}, fmt.Errorf("GitHub 文件响应缺少 sha")
	}
...
// putGitHubFile creates or updates a file in the GitHub repository via the Contents API.
