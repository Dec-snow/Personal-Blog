...
import { galleryAlbums as staticAlbums } from "../data/galleryAlbums";
...
const GalleryPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled && data.albums && data.albums.length > 0) {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#fff8f1] pt-24 text-[#241322]">
      ...
      {loading ? (
        <p className="text-center text-[#8a7078] text-sm py-12">加载中…</p>
      ) : albums.length === 0 ? (
        <p className="text-center text-[#8a7078] text-sm py-12">暂无相册，去后端控制台发布吧。</p>
      ) : (
        <section className="container mx-auto grid gap-6 px-5 pb-24 md:grid-cols-2 md:px-10">
            ...
            >
              ...
              <div className="relative z-10 mt-5">
                <p className="font-general text-[10px] uppercase tracking-[0.36em] text-[#b76e79]">
                  {String(album.images?.length || 0).padStart(2, "0")} Photos
                </p>
