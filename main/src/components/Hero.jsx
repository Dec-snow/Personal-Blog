...
const COS = cosAsset("");
const HERO_IMAGE_EXTS = ["jpg", "jpg", "jpg", "jpg"];
const getImgSrc = (index) =>
  `${COS}/壁纸/0${index}.${HERO_IMAGE_EXTS[index - 1]}`;
...
const Hero = () => {
  ...
  useEffect(() => {
    let cancelled = false;
    // Priority images (1 & 2) load immediately — needed for first paint.
    // Secondary images (3 & 4) defer until after first paint via requestIdleCallback.
    const priorityIndexes = HERO_INDEXES.filter((i) => i <= 2);
    const deferredIndexes = HERO_INDEXES.filter((i) => i > 2);
    ...
    priorityIndexes.forEach(loadImage);

    const ric = window.requestIdleCallback || ((cb) => setTimeout(cb, 1200));
    const handle = ric(() => {
      if (cancelled) return;
      deferredIndexes.forEach(loadImage);
    });
  ...
  return (
    <div className="relative h-dvh w-screen overflow-hidden">
      ...
      >
              ...
              >
                ...
                >
                  <div
                    ...
                    style={{
                      background:
                        "radial-gradient(circle at 50% 43%, #ff8fab 0%, #b76e79 40%, #4a2c6e 100%)",
                    }}
                  />
        ...
        <h1 className="special-font hero-heading absolute bottom-5 right-5 z-40 text-blue-75 !text-2xl sm:!text-3xl md:!text-5xl lg:!text-6xl">
          追著猫的<b>老</b>鼠
        </h1>
      ...
      <h1 className="special-font hero-heading absolute bottom-5 right-5 text-black !text-2xl sm:!text-3xl md:!text-5xl lg:!text-6xl">
        追著猫的<b>老</b>鼠
      </h1>
    </div>
  );
