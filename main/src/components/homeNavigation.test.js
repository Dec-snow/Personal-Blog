...
test("keeps the moments entry in the top navbar", () => {
  ...
  assert.match(navbarSource, /\{\s*label:\s*"随笔",\s*to:\s*"\/moments"/);
});
...
test("keeps the about preview production-safe", () => {
  ...
  assert.match(aboutPreviewSource, /追著猫的老鼠/);
...
test("keeps the about projects grid roomy in the integrated site", () => {
  ...
  assert.match(aboutPreviewSource, /\.projects\s*\{\s*display:\s*grid/);
  assert.match(aboutPreviewSource, /max-width:\s*26rem/);
});
...
test("keeps the AI data-center layout compact on phones", () => {
  ...
  assert.match(aiTrafficSource, /ServerInfoPanel/);
...
test("names the moments page 随笔", () => {
  const momentsPageSource = readSource("pages/MomentsPage.jsx");

  assert.match(momentsPageSource, /<h1>随笔<\/h1>/);
  assert.doesNotMatch(momentsPageSource, /<h1>说说<\/h1>/);
  assert.doesNotMatch(momentsPageSource, /<h1>碎语<\/h1>/);
});
...
test("themes the moments footer for its page palette", () => {
