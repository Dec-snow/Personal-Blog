...
// 当前使用静态图片替代视频。如需切换回视频：
// 1. import LazyVideo from "./LazyVideo";
// 2. 将 MediaImage 替换回 LazyVideo
// 3. 将 src 改为视频路径（如 ${COS}/videos/feature-1.mp4）
const COS = cosAsset("");

const MediaImage = ({ src, priority = false, className = "" }) => {
    ...
    return (
      ...
      >
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, #1a1330 0%, #2d1b4e 40%, #4a2c6e 70%, #6b3a8e 100%)",
          }}
        />
      </div>
    );
...
const ARCHIVE_ITEMS = [
  ...
  },
  {
    ...
    title: "安静影像",
    subtitle: "片段四",
    src: `${COS}/Home/片段04.jpg`,
    note: "一枚适合停下来多看一会儿的安静影像碎片。",
  },
  {
    index: "06",
    title: "More coming soon.",
    subtitle: "Feature Six",
    src: `${COS}/Home/片段06.jpg`,
  ...
  if (visualOnly) {
    return (
      <div className="group relative size-full overflow-hidden">
        <MediaImage src={src} priority={videoPriority} />
  ...
  return (
    <div className="group relative size-full overflow-hidden">
      ...
      <div className="absolute inset-0 z-20 flex flex-col justify-between p-4 text-blue-50 md:p-5">
        ...
        {linkUrl && (
          (() => {
            const isInternal = String(linkUrl).startsWith("/");
            const linkClass = "w-fit";
            const linkProps = isInternal ? {} : { target: "_blank", rel: "noopener noreferrer" };
            const Wrapper = isInternal ? Link : "a";
            const wrapperProps = isInternal ? { to: linkUrl } : { href: linkUrl };
            return (
              <Wrapper
                className={linkClass}
                {...wrapperProps}
        ...
        <div className="relative min-h-[76vh] overflow-hidden rounded-[2rem] border border-[#f6d8c8]/60 bg-[linear-gradient(120deg,#fff8ef,#ffe9df)] shadow-[0_34px_120px_rgba(0,0,0,0.38)]">
          ...
          <div className="grid h-full min-h-[76vh] md:grid-cols-[1.7fr_0.42fr]">
              ...
              <div className="relative h-[62vh] w-full max-w-6xl [transform-style:preserve-3d]">
                {ARCHIVE_ITEMS.map((item, itemIndex) => {
                  ...
                  return (
                    ...
                    >
                      <MediaImage src={item.src} priority={isActive} />
            ...
            <div className="relative flex flex-col justify-center border-t border-[#ead4bf] bg-white/46 p-5 md:border-l md:border-t-0 md:p-5">
              ...
              {activeItem.linkUrl && (
                (() => {
                  const isInternal = String(activeItem.linkUrl).startsWith("/");
                  const Wrapper = isInternal ? Link : "a";
                  const wrapperProps = isInternal
                    ? { to: activeItem.linkUrl }
                    : { href: activeItem.linkUrl, target: "_blank", rel: "noopener noreferrer" };
                  return (
                    <Wrapper
                      {...wrapperProps}
