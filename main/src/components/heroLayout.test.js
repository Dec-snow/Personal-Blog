...
test("renders the hoarfrost wordmark inside the hero with the current wallpaper index", () => {
  assert.match(heroSource, /import HeroWordmark from "\.\/HeroWordmark"/);
  assert.match(heroSource, /<HeroWordmark heroIndex=\{currentIndex\} \/>/);
});

test("draws the hoarfrost wordmark from generated Pacifico glyph outlines", () => {
...
test("keeps the hoarfrost wordmark out of the main phone hero content", () => {
